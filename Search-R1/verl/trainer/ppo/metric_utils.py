...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # ===== 自定义指标：multi_turn_call（原 second_score） =====
    if "all_second_scores" in batch.non_tensor_batch:
        second_scores = torch.tensor(
            batch.non_tensor_batch["all_second_scores"], 
            dtype=torch.float32
        )
        # 只统计非 aborted 样本（和 reward 保持一致）
        non_aborted_mask = ~aborted_mask
        valid_second = second_scores[non_aborted_mask]

        if valid_second.numel() > 0:
            metrics.update({
                "reward/multi_turn_call/mean": torch.mean(valid_second).item(),
                "reward/multi_turn_call/max":  torch.max(valid_second).item(),
                "reward/multi_turn_call/min":  torch.min(valid_second).item(),
            })
    return metrics
